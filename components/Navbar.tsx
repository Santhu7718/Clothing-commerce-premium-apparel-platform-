...
export const Navbar: React.FC<NavbarProps> = ({ 
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] flex items-center justify-between px-6 py-4 md:py-6 transition-all duration-500 glass-panel md:px-16 border-b border-white/[0.05]">
      {/* Left Links */}
      ...
      {/* Center Brand - Ensuring no overlap */}
      <div className="absolute left-1/2 -translate-x-1/2 pointer-events-none md:pointer-events-auto">
        <h1 
          onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})}
          className="text-lg md:text-2xl font-serif-premium tracking-[0.6em] font-bold text-white cursor-pointer select-none text-glow text-center whitespace-nowrap"
        >
          THE HUMAN CLUB
        </h1>
      </div>
      ...
      <div className="flex items-center gap-2 md:gap-5">
        <div className={`flex items-center transition-all duration-700 overflow-hidden ${isSearchOpen ? 'w-32 md:w-64 opacity-100' : 'w-0 opacity-0'}`}>
          <input 
            ...
            className="bg-transparent border-b border-premium-gold/30 text-[10px] tracking-[0.3em] w-full py-2 focus:outline-none placeholder:text-zinc-800 uppercase text-white"
          />
        </div>
        
        <button onClick={() => setIsSearchOpen(!isSearchOpen)} className="p-2 hover:bg-white/10 rounded-full transition-all">
          {isSearchOpen ? <X className="w-5 h-5 text-premium-gold" /> : <Search className="w-5 h-5" />}
        </button>

        <button onClick={openWishlist} className="relative p-2 hover:bg-white/10 rounded-full transition-all group">
          <Heart className={`w-5 h-5 group-hover:text-premium-gold transition-colors ${wishlistCount > 0 ? 'fill-premium-gold text-premium-gold' : ''}`} />
          {wishlistCount > 0 && (
            <span className="absolute top-0 right-0 flex items-center justify-center w-3.5 h-3.5 text-[8px] font-bold text-black bg-white rounded-full">
              {wishlistCount}
            </span>
          )}
        </button>

        <button onClick={openCart} className="relative p-2 hover:bg-white/10 rounded-full transition-all group">
          <ShoppingBag className="w-5 h-5 group-hover:text-premium-gold transition-colors" />
          {cartCount > 0 && (
            <span className="absolute top-0 right-0 flex items-center justify-center w-3.5 h-3.5 text-[8px] font-bold text-black bg-premium-gold rounded-full">
              {cartCount}
            </span>
          )}
        </button>
        ...
        <div className="relative ml-1 md:ml-2">
          <button 
            ...
            className="p-1 hover:bg-white/10 rounded-full transition-all flex items-center gap-2 border border-white/5 pr-3 pl-1"
          >
            {user?.avatar ? (
              <img src={user.avatar} className="w-7 h-7 rounded-full border border-premium-gold/30" alt="Avatar" />
            ) : (
              <div className={`p-1.5 rounded-full ${user ? 'bg-premium-gold/20 text-premium-gold' : 'bg-white/5'}`}>
